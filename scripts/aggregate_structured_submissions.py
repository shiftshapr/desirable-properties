...
def get_structured_submissions_path():
    ...
    # Go up one level to meta-layer, then to submissions/structured
    structured_path = script_dir.parent / "submissions" / "structured"
...
def aggregate_submissions():
    ...
    for file in structured_path.glob("*.json"):
        # Skip any non-submission files
        if file.name not in ['.DS_Store']:
