...
PROCESSED_EML_DIR = os.path.join(EML_DIR, "processed")
PROCESSED_TXT_DIR = os.path.join(TXT_DIR, "processed")

os.makedirs(OUTPUT_DIR, exist_ok=True)
os.makedirs(PROCESSED_EML_DIR, exist_ok=True)
...
# Extract full HTML or plain text from multipart email
    ...
    if msg.is_multipart():
        for part in msg.walk():
            ...
            if content_type == 'text/html':
                ...
                fallback = part.get_content()
        return fallback if 'fallback' in locals() else ""
    else:
        return html_to_text(msg.get_content()) if msg.get_content_type() == 'text/html' else msg.get_content()
...
for filename in os.listdir(EML_DIR):
    if filename.endswith(".eml"):
        ...
        shutil.move(path, os.path.join(PROCESSED_EML_DIR, filename))
