...
Staging sets `DP_PUBLIC_BASE=https://staging.desirableproperties.org` in `ecosystem.staging.config.js`. Override via `.env.local` key `DP_PUBLIC_BASE_STAGING` if needed.
