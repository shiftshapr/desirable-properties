...
export async function POST(request: Request, { params }: RouteParams) {
  ...
  if (!body.userSummary || !body.bodyMarkdown) {
    return NextResponse.json({ error: 'userSummary and bodyMarkdown required' }, { status: 400 });
  }

  const upstream = await fetch(
    `${getHermesChatUrl()}/api/hermes/threads/${encodeURIComponent(id)}/contribution-record`,
    {
      method: 'POST',
