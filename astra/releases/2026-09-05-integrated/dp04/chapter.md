...
*Your data, your terms: meaningful control over use, sharing, and protection, with visible limits and accountable follow-through.*
...
Failed data sovereignty can also encourage retreat into silos, fabricated accounts, or manufactured activity while extraction continues. That response can damage the social signals on which others rely; privacy protection should not depend on fabricating a digital presence. Pseudonymity itself is not evidence of deception.

### 3.1 Consent theater

A person may agree to one use of their data without realizing that the same action authorizes several unrelated uses.
...
Information collected for one purpose is later used for another without giving the participant a meaningful choice about the change.
...
A person can download their data but cannot use it to continue their activities elsewhere.
...
Systems create consequential information about people from their behavior, including information they never directly supplied, without clear rules for its creation, use, challenge, or removal.
...
Data can remain long after the reason for keeping it has ended. Deleting an account may leave related copies and derived information in other systems.
...
Information from different parts of a person’s life is quietly linked through identifiers, device graphs, or fingerprinting, even when the person expected those activities to remain separate.
...
Removing names does not necessarily make a dataset anonymous. Routines, context, or outside knowledge may still allow someone to identify the people it describes.
...
Deleting, correcting, or withdrawing permission in one service does not necessarily change the copies held by its partners.
...
Different parts of a person’s life can be combined despite their deliberate separation. Shared infrastructure, account-level integrations, or an undifferentiated vault can expose information from one persona to another.
...
These conditions form a coordinated system of protections. Consent without useful portability can still create lock-in; deletion without propagation leaves downstream copies unresolved; transparency without accountability leaves participants unable to act on what they learn. Each mechanism protects a different aspect of control, and partial compliance is insufficient when another mechanism fails.
...
Every collection and processing pathway must state why information is needed in terms participants and communities can understand. A material change in purpose requires visible reauthorization, reclassification, or zone-level review before the new use begins.
...
Systems must collect, retain, and share only the information needed for the function being offered. Additional collection must follow visible, justified choices rather than become the default merely because the system can collect more.
...
Participants must be able to approve, decline, and later change permissions for distinct categories of data use separately. Agreeing to a wanted feature should not require blanket permission for unrelated uses.
...
Portability must let people and communities continue their activities elsewhere, preserving the information, context, and relationships that make their data useful rather than merely providing downloadable files.
...
People should be able to see how long their information will be kept and what happens after they request deletion, correction, or withdrawal of permission. Retention must be bounded by events or stated purposes, and requests must propagate to known downstream systems with auditable outcomes.
...
Information a system infers about a person can be more consequential than the information they supplied. High-risk inferences therefore require stronger protections, including meaningful opportunities to understand, challenge, and limit their use.
...
Communities must be able to set and enforce stronger privacy rules for the spaces they govern rather than rely only on platform defaults. These rules should reflect the community’s context while remaining interoperable with the broader infrastructure.
...
Profiles should be evaluated against concrete harms to marginalized participants, including LGBTQ+ erasure or unwanted disclosure through automated moderation and cross-context inference. Relevant identity and context must not disappear merely because a system applies a generic category of vulnerability.

Profiles may use privacy-preserving lookup and attestation methods suited to the task. Anonymizing proxies and hash-prefix lookups reduce particular disclosures but are not, for that reason alone, zero-knowledge proofs. Civic participation can instead use appropriate zero-knowledge proofs or selective disclosure to attest a limited fact; real-time presence and support signals still need participant control and scrutiny for re-identification in small groups.

### 5.8 Auditability and provenance of use

Participants and communities must be able to understand significant uses of their data: what happened, why it happened, and who or what performed the action. Records of access, transfer, and inference should make those questions answerable.
...
Allowing others to read or search a contribution does not by itself authorize its use to train or improve an AI system. Participants and communities should be able to distinguish those permissions without losing unrelated participation capabilities.

### 5.10 Jurisdictional and transfer honesty

People and communities must be able to see when their data crosses into another legal jurisdiction and understand how that affects applicable protections and routes for redress.
...
A coherent data system layer must preserve **lineage, semantics, and permissions** across pipelines, services, and time. In practical terms, information should retain where it came from, what it means, what changed, and which uses remain authorized as people or AI transform and move it. Those claims must remain verifiable under scale and adversarial use.
...
### 5.12 Private community coordination

Communities should be able to coordinate through encrypted group communication and participant- or community-controlled hosting, reducing dependence on a single platform for their data and continuity. For high-stakes disclosure, including whistleblowing, tools should support deliberate control of identity exposure, storage, and disclosure timing.

Deletion or anonymization requests should be available within the systems’ actual control, with explicit limits once information has been shared or copied. Encrypted transport and decentralized storage are useful mechanisms, not guarantees that every adversary or downstream recipient loses access. DP15 addresses the threat model and DP18 closes the reporting feedback loop.

---

## 6. Data Vaults and Personas
...
These structures preserve familiar social boundaries. A person may share different information with a healthcare provider, an employer, a neighborhood association, and a support group. The vault provides control over custody and use; personas govern what is presented and linkable in each setting. Digital participation should not require those contexts to collapse into one profile.

### 6.1 The personal data vault
...
A vault can also support participant-directed AI activity rather than act only as a passive container. The VeMe proposal illustrates a vault-as-agent pattern in which memory, outputs, and inferences remain governed by participant choices and the vault’s existing consent, purpose, and retention controls.
...
### 6.2 Personas as context boundaries
...
Personas should support anonymous, pseudonymous, or authenticated interactions where the relevant zone permits them. Authentication should not automatically disclose a public identity or link other personas. Connected-device identities also need contextual boundaries, since a device can otherwise become the bridge that silently reconnects separated activity.
...
### 6.4 Relationship to identity and naming
...
### 7.1 Personal privacy intermediaries

A participant-configured agent can mediate interactions with networks, applications, forms, and connected devices. It can help apply permissions to differently sensitive flows, filter incoming material according to stated preferences, and restrict device interactions to networks the participant trusts. Its actions and filters should remain visible, inspectable, and revocable.

A browser or embedded data lens can complement that intermediary by showing collection and tracking while an interaction is happening. The lens should distinguish observed activity from inferred activity, so participants can act on useful evidence without mistaking the tool for a complete inventory of hidden processing.
...
## 9. Community Signals Informing DP4
...
## 13. Minimum DP4 Alignment (Non-Normative)
...
### 13.1 Purpose binding and enforcement
...
### 13.5 Retention and propagation discipline
...
### 15.1 Privacy transport and civic sandbox trials

A decentralized privacy-routing mesh is one implementation option for reducing exposure of interaction, contribution, and identity-resolution traffic. Multiparty relays and onion routing are candidate approaches. Trials should keep routing optional and evaluate protection, operator dependencies, accessibility, and performance before wider adoption. DP7 addresses compatibility with participant-chosen privacy infrastructure, and DP17 addresses possible relay funding.

Local, multi-sector civic sandboxes, including the proposed UMi Insecurity Initiatives and Sandbox environments, can test whether privacy mechanisms are understandable, equitable, and effective in practice. A pilot’s institutional setting is evidence to examine, not proof of the mechanism’s success.
